# -*- coding: utf-8 -*-
"""
🏗️ ORQUESTRATOR ISA v6.0-STABLE
Stack: FastAPI + SQLAlchemy 2.0 (psycopg3 async) + Neon DB + WhatsApp Cloud API
Python 3.10 | Render | Single File | Production Ready
"""
...
from sqlalchemy import Enum as SAEnum, String, Text, Integer, Boolean, DECIMAL, Date, Time, DateTime, select
...
if DATABASE_URL:
    ...
    engine = create_async_engine(
        ...
        pool_recycle=300,         # ✅ Cierra/reabre cada 5 min (evita timeout de Neon)
        echo=False
    )
...
# ==========================================================
# 📦 MODELOS SQLALCHEMY 2.0 (SINTAXIS LIMPIA)
# ==========================================================
# 3. ENUMS CORRECTOS (Saltos de línea reales, NO comas)
...
# ==========================================================
# 🤖 BOT LOGIC (FLUJO LINEAL + PERSISTENCIA EN DB)
# ==========================================================
async def process_msg(payload: dict):
    ...
    try:
        ...
        async with async_session_maker() as db:
            ...
            # Conversación (Estado persistente)
            ...
            if ctx.get("fase") == "lang" or txt in ("q", "salir"):
                if txt == "1": reply="🇪🇸 Español. Envía `m` para menú."; ctx["lang"]="es"
                elif txt == "2": reply="🇬🇧 English. Send `m`."; ctx["lang"]="en"
...
# 🌐 APP & ROUTES
# ==========================================================
app = FastAPI(title="Orquestrator ISA v6.0")
