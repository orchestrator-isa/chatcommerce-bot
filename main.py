    ...
    for entry in body.get("entry", []):
        for change in entry.get("changes", []):
            ...
            for msg in val.get("messages", []):
                ...
                fase = pedido_estado.get(user_id, {}).get("fase", "inicio") # FASE SE CREA AQUÍ

                # 2️⃣ COMANDO Q / SALIR (Re<think>
</think>

Bro, te lo explico directo y sin vueltas para que no tires tu dinero:

### 💰 ¿Qué es eso de "Pipeline Minutes PAYG ($5 una vez)"?
Es un bloqueo de seguridad de Render. Como estás en el plan **Free**, Render te da una cantidad limitada de minutos para "construir" (compilar) tu código cada mes.
El error dice: *"You've used up the maximum number of pipeline minutes"*.
Significa que **se te acabó el tiempo de compilación del mes** (o el límite de $0 de tu cuenta).

### 🚦 ¿Debes pagarlo?
**SÍ**, si quieres que el botón de "Deploy" vuelva a funcionar **HOY**. Si no pagas (o no aumentas tu límite de gasto), Render rechazará cualquier `git push` que hagas hasta el próximo mes.

### ⚠️ PERO ESPERA: ¡NO PAGUES TODAVÍA!
Si pagas los $5 ahora y subes tu código actual, **vas a perder el dinero**.
¿Por qué? Porque tus logs muestran que el código actual tiene errores de sintaxis graves:
1.  `IndentationError` (línea 337): El código está mal indentado.
2.  `UnboundLocalError: 'fase'`: El bot se cae al recibir mensajes.

Si pagas ahora, Render usará esos minutos en intentar compilar, encontrará esos errores y **fallará de nuevo**.

---

### 🛠️ PLAN DE ACCIÓN (Haz esto en orden estricto)

#### 1. Arregla el `main.py` en tu máquina
Reemplaza **TODO** el contenido de tu `main.py` local con este código definitivo. Está corregido, indentado perfectamente y sin bloques duplicados.
