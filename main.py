#!/usr/bin/env python3
"""
Orquestrator ISA — ChatCommerce Bot v2.0
...
# ══════════════════════════════════════════════════════════
# NLP — 7 idiomas
# Darija (Tetouan), Arabe, Frances, Espanol, Ingles, Aleman, Turco
# ══════════════════════════════════════════════════════════
class DarijaNLP:
    ...
    def detect_language(cls, text: str) -> str:
        ...
        if arabic_chars > len(text) * 0.2:
            return "arabic"
        for lang in ["german", "turkish", "french", "spanish", "english"]:
            if any(kw in text_lower for kw in cls.LANG_KEYWORDS[lang]):
                return lang
        return "darija"
...
# ══════════════════════════════════════════════════════════
# BOT LOGIC — Respuestas en 7 idiomas
# ══════════════════════════════════════════════════════════
class BotLogic:
...
async def lifespan(app: FastAPI):
    logger.info("[STARTUP] Orquestrator ISA v2.0 iniciando...")
    try:
        sb = get_supabase()
...
async def create_client(client: ClientCreate):
    try:
        sb = get_supabase()
        data = {
            "name": client.name, "owner_phone": client.owner_phone,
            "language": client.language, "business_type": client.business_type,
            "zone": client.zone, "plan": client.plan,
...
async def create_menu_item(item: MenuItemCreate):
    try:
        ...
        res = sb.table("menu_items").insert({
            "client_id": item.client_id, "category": item.category,
            "dish_name": item.dish_name, "description": item.description,
            "price": item.price, "is_available": True,
        }).execute()
...
async def get_stats():
    try:
        ...
        return {
            "clients": clients.count, "menu_items": menu.count,
