...
# ruff: noqa: E501
"""
ORQUESTRATOR ISA v19.0 - FLUJO DE ENTREGA Y PAGO
- Confirmar pedido: pregunta tipo de entrega, dirección (si domicilio), método de pago
- Tiempo estimado dinámico (platos + pedidos activos)
- Tarjeta solo permitida en recogida
- Sin billete, sin cambio, sin transferencia
- Mantiene todas las funcionalidades anteriores (idiomas, menú, carrito, reservas)
"""
...
# APP
# ============================================================
app = FastAPI(title="Orquestrator ISA v19.0")
...
# ============================================================
# TRADUCCIONES (mensajes de entrega y pago añadidos)
# ============================================================
I18N = {
    "es": {
        "welcome": (
        ...
        "direccion_request": "📍 Por favor, escribe tu dirección completa:\n(Calle, número, referencia, etc.)",
        "payment_method": "💳 *Método de pago*\n1. Efectivo\n2. Tarjeta (Visa/Mastercard)\n\nResponde con el número:",
        "card_not_allowed_delivery": "❌ Lo siento, el pago con tarjeta solo está disponible para recogida en local.\nElige *1* para pagar en efectivo.",
        "order_confirmed": (
            "✅ *Pedido confirmado!*\n"
            "📋 Número: #{numero}\n"
            "🚚 Tipo: {delivery_type}\n"
        ...
        # ... resto de claves (reservas, etc.) igual que antes
    ...
    "en": {
        ...
        "direccion_request": "📍 Please enter your full address:\n(Street, number, reference, etc.)",
        "payment_method": "💳 *Payment method*\n1. Cash\n2. Card (Visa/Mastercard)\n\nReply with the number:",
        "card_not_allowed_delivery": "❌ Sorry, card payment is only available for pick up.\nChoose *1* to pay in cash.",
        "order_confirmed": (
            "✅ *Order confirmed!*\n"
            "📋 Number: #{numero}\n"
            "🚚 Type: {delivery_type}\n"
...
# ============================================================
# BOT: PROCESAMIENTO DE MENSAJES (con flujo de entrega y pago)
# ============================================================
async def process_msg(payload: dict):
    ...
    try:
        ...
        async with async_session_maker() as db:
            ...
            ctx.setdefault(
                "pedido_temp", {}
            )  # para almacenar datos temporales del flujo de pedido
            ...
            if txt in ("q", "salir", "quit"):
                try:
                    ...
                    await db.commit()
                    logger.info(f"✅ q ejecutado para {cli.wa_id}")
                except Exception as e:
            ...
            # ========== FASES DE PEDIDO (entrega, direccion, pago) ==========
            if fase == "entrega":
                if txt == "1":
                    ...
                    ctx["fase"] = "direccion"
                    reply = t("direccion_request", lang)
                else:
                    reply = t("delivery_type", lang)

            elif fase == "direccion":
                if txt_raw.strip():
                    ctx["pedido_temp"]["direccion"] = txt_raw.strip()
                    ctx["fase"] = "pago"
                    reply = t("payment_method", lang)
                else:
                    reply = t("direccion_request", lang)
                ...
                if txt == "1":  # efectivo
                    ...
                    # Limpiar carrito y contexto temporal
                    ctx["carrito"] = []
                    ctx["pedido_temp"] = {}
                    ctx["fase"] = "menu"
                    await db.commit()
                    ...
                    if tipo_entrega == "domicilio":
                        reply = t("card_not_allowed_delivery", lang)
                        # no avanzamos, se queda en fase pago
                    else:
                        ctx["pedido_temp"]["metodo_pago"] = "tarjeta"
                        items = ctx.get("carrito", [])
                        if not items:
                            reply = t("confirm_empty", lang)
                            ctx["fase"] = "menu"
            ...
            # ========== FLUJO NORMAL (menú, carrito, reservas) ==========
            elif fase == "lang":
                ...
                if new_lang:
                    ...
                    await db.flush()
                    # No mostrar menú automáticamente, solo bienvenida con comandos
                ...
                # Cantidades: "5 33"
                    ...
                    if items:
                        ...
                        ctx["fase"] = "entrega"
                        ctx["pedido_temp"] = {}  # reset temporal
                        reply = t("delivery_type", lang)
                    else:
                        reply = t("confirm_empty", lang)
...
# ============================================================
# PANEL HTML (igual que antes, sin cambios)
# ============================================================
