...
if DATABASE_URL:
    ...
    logger.info("✅ Engine DB inicializado")

class Base(DeclarativeBase): pass

# 📦 MODELOS (ENUMS CORRECTOS CON SALTOS DE LÍNEA)
...
async def process_msg(payload: dict):
    ...
    try:
        ...
        async with async_session_maker() as db:
            ...
            if not cli:
                cli = Cliente(id_restaurante=rest.id_restaurante, wa_id=phone, telefono=phone)
            ...
            if not conv:
                conv = Conversacion(id_cliente=cli.id_cliente, id_restaurante=rest.id_restaurante, contexto_bot={"fase":"lang", "carrito":[]})
                db.add(conv)
            ...
            if txt == "m":
                ...
                if txt in platos:
                    ...
                    await send_wa(phone, f"✅ {platos[txt]['n']} añadido. Total: {total} MAD.")
            elif txt == "c":
                if ctx.get("carrito"):
                    total = sum(i['p'] for i in ctx["carrito"])
                    ped = Pedido(id_restaurante=rest.id_restaurante, id_cliente=cli.id_cliente, items=ctx["carrito"], total=Decimal(str(total)))
                    db.add(ped)
                    await db.commit()
...
# 🌐 APP & ROUTES (CORREGIDAS)
app = FastAPI(title="Orquestrator ISA v4.2")
...
async def wb_post(req: Request, bg: BackgroundTasks):
    try:
        ...
        # FIX: JSONResponse necesita 'content'
...
# --- PANEL HTML (RUTAS SEPARADAS CORRECTAMENTE) ---
...
HTML_RECEPCION = """<html><body class="bg-gray-50 p-6"><h1>📊 Recepción</h1><div>Cargando...</div></body></html>"""
